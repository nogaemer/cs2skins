...
import database.postgres.CatalogRepository.ItemUpsertRow
import database.postgres.CatalogRepository.ItemWearRow
...
class PostgresSeedService(
    ...
    suspend fun seedSkins() {
        ...
        val itemRowsByMarketHashName = linkedMapOf<String, ItemUpsertRow>()
        ...
        fun addItemWithWears(item: Item, collectionId: Long, weaponId: String, rarityId: Short, json: JSONObject) {
            ...
            itemRowsByMarketHashName[item.marketHashName] = ItemUpsertRow(item, collectionId, weaponId, rarityId)
        ...
        val wearRows = itemRows.flatMap { row ->
            ...
            wearCodesByItemId[itemId]?.map { code -> ItemWearRow(itemId, code) } ?: emptyList()
        }
