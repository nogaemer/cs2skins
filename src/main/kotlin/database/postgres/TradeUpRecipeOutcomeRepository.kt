...
class TradeUpRecipeOutcomeRepository(
    ...
    /**
     * Upserts many recipe-outcome rows in as few round-trips as possible.
     * theoretical_probability is a static property of a recipe (independent of price or
     * run time), so this is safe to call repeatedly across runs — later calls simply
     * refresh the same rows via ON CONFLICT.
     */
    fun upsertOutcomesBatch(outcomes: List<OutcomeInput>) {
        ...
        distinctOutcomes.chunked(3000).forEach { chunk ->
            upsertChunk(chunk)
        }
    }
    ...
    private fun upsertChunk(outcomes: List<OutcomeInput>) {
        ...
        val sql = """
            ...
            VALUES $valuesSql
            ON CONFLICT (tradeup_recipe_id, outcome_item_id) DO UPDATE SET
        ...
        dataSource.connection.use { conn ->
            conn.prepareStatement(sql).use { statement ->
                ...
                outcomes.forEach { outcome ->
                    statement.setLong(index++, outcome.tradeUpRecipeId)
